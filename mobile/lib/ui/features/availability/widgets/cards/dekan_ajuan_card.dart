// File: dekan_ajuan_card.dart
// Deskripsi: Widget kartu pengajuan mengajar dosen untuk role Dekan (Level 2 Approval).
...
class DekanAjuanCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (isConflict) {
      ...
      statusLabel = 'Menunggu Dekan';
    } else if (aj.status.startsWith('ditolak')) {
      statusColor = const Color(0xFFDC2626);
      statusBg = const Color(0xFFFEF2F2);
      statusLabel =
          aj.status == 'ditolak_dekan' ? 'Ditolak Dekan' : 'Ditolak';
    } else {
                        ...
                        onPressed: () {
                          AjuanPengajaranDialogs.showRejectDialog(
                            ...
                            onConfirmReject: (alasan) async {
                              await api.dekanRejectAjuan(aj.id, alasan);
                              if (context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text(
                                      'Ajuan dikembalikan ke KaProdi.',
                                    ),
                    ...
                    Expanded(
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          ...
                          await api.dekanApproveAjuan(aj.id);
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  'Ajuan disetujui & diteruskan ke Admin!',
                                ),
                                backgroundColor: AppColors.primary,
                              ),
                            );
